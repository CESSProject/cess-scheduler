...
var char_base58 = []byte("123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz")
...
func Base58Encoding(str string) string {
	...
	for strTen.Cmp(big.NewInt(0)) > 0 {
		...
		modSlice = append(modSlice, char_base58[mod.Int64()])
	}
...
func Base58Decoding(str string) string {
	...
	for _, byteElem := range strByte {
		index := bytes.IndexByte(char_base58, byteElem)
